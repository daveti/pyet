...
# 2014-07-04 daveti	added line number support for Element (hack)
#
...
class Element(object):
    ...
    #daveti: hack!
    # Line number from the original xml file
    lineNum = -1
    ...
    def __init__(self, tag, attrib={}, **extra):
        ...
        self._children = []
    '''
    def __init__(self, lineNum, tag, attrib={}, **extra):
        attrib = attrib.copy()
        attrib.update(extra)
    ...
    #daveti: new make with line number
    '''
    def makeelement(self, tag, attrib):
        return self.__class__(tag, attrib)
    '''
    def makeelement(self, lineNum, tag, attrib):
	return self.__class__(lineNum, tag, attrib)
	...
	'''
	elem = self.makeelement(self.lineNum, self.tag, self.attrib)
        elem.text = self.text
        elem.tail = self.tail
        elem[:] = self
        return elem
    ...
    #daveti: new helper for line number
    def getLineNum(self):
	return self.lineNum
...
# daveti: new sub with line number
'''
def SubElement(parent, tag, attrib={}, **extra):
    ...
    return element
'''
def SubElement(parent, lineNum, tag, attrib={}, **extra):
    attrib = attrib.copy()
    attrib.update(extra)
    element = parent.makeelement(lineNum, tag, attrib)
    parent.append(element)
    return element
...
class TreeBuilder(object):
    ...
    # @defreturn Element
    #daveti: udpate start with line number
    '''
    def start(self, tag, attrs):
        ...
        return elem
    '''
    def start(self, lineNum, tag, attrs):
        self._flush()
        self._last = elem = self._factory(lineNum, tag, attrs)
        if self._elem:
            self._elem[-1].append(elem)
        self._elem.append(elem)
        self._tail = 0
...
class XMLParser(object):
	...
	#daveti: inject the line number
	'''
        return self.target.start(tag, attrib)
	'''
	return self.target.start(self._parser.CurrentLineNumber, tag, attrib)
